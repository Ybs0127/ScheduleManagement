...
namespace {
/**
 * @brief 일정 내용을 읽기 전용으로 보여주는 상세 확인 다이얼로그이다.
 *
 * ScheduleListWidget에서 일정 카드를 더블클릭했을 때 사용되며,
 * 일정의 시간, 제목, 메모를 수정 없이 검토할 수 있도록 표시한다.
 */
...
/**
 * @brief 일정 추가 및 수정 입력을 담당하는 편집 다이얼로그이다.
 *
 * ScheduleListWidget에서 새 일정을 만들거나 기존 일정을 수정할 때 사용되며,
 * 시작/종료 일시, 제목, 메모를 입력받아 하나의 ScheduleItem으로 반환한다.
 *
 * 주요 변수 설명:
 * - m_item: 편집 중인 일정 데이터이며 저장 시 입력값으로 갱신된다.
 * - m_startDateTimeEdit: 시작 날짜/시간 입력 위젯이다.
 * - m_endDateTimeEdit: 종료 날짜/시간 입력 위젯이다.
 * - m_titleEdit: 일정 제목 입력 위젯이다.
 * - m_descriptionEdit: 일정 메모 입력 위젯이다.
 *
 * 주요 함수 설명:
 * - item(): 사용자가 저장한 결과 일정을 호출자에게 반환한다.
 */
...
private:
    /// 저장 버튼 승인 시 최신 입력값이 반영되는 편집 대상 일정 데이터이다.
    ScheduleItem m_item;
    /// 일정 시작 일시를 입력받는 위젯이다.
    QDateTimeEdit *m_startDateTimeEdit;
    /// 일정 종료 일시를 입력받는 위젯이다.
    QDateTimeEdit *m_endDateTimeEdit;
    /// 일정 제목을 입력받는 라인 에디트이다.
    QLineEdit *m_titleEdit;
    /// 일정 메모를 입력받는 멀티라인 텍스트 에디트이다.
